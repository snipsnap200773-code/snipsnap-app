  ...
  return (
    <div className="invoice-standalone-root" style={{ width: '100%', minHeight: '100vh', backgroundColor: '#f1f5f9' }}>
      <style>{`
        ...
        .screen-ui { padding: 40px 20px; max-width: 700px; margin: 0 auto; }
        ...
        .toggle-btn { padding: 14px; border: 1px solid #cbd5e1; border-radius: 12px; background: white; cursor: pointer; font-weight: bold; }
        .toggle-btn.active { background: #1e3a8a; color: white; border-color: #1e3a8a; }
      ...
      {isPreviewOpen && (
        <div className="modal-overlay" onClick={() => setIsPreviewOpen(false)}>
          <div className="preview-header no-print" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ color: 'white', fontWeight: 'bold' }}>📄 請求明細書 プレビュー</div>
                ...
                <div key={pageIdx} className="full-list-page">
                  {pageIdx === 0 && (
                    <header style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #000', marginBottom: '15px', paddingBottom: '5px' }}>
                      <div>
                        {/* 🌟 ○○月度 をタイトルに追加 */}
                        <h1 style={{ margin: 0, fontSize: '24px' }}>{titleDateStr} 請求明細書</h1>
                        <h2 style={{ margin: 0, fontSize: '18px' }}>{myFacilityName} 御中</h2>
                      </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                    ...
                    <tbody>
                      {sortedList.slice(pageIdx * itemsPerPage, pageIdx * itemsPerPage + itemsPerPage).map((item, idx) => (
